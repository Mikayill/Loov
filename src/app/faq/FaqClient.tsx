...
function AccordionItem({ q, a }: { q: string; a: string }) {
  ...
  return (
    <div className="border-b border-[#F0E8E0] last:border-0">
      ...
      >
        <span className="font-semibold text-ink text-sm group-hover:text-accent transition-colors leading-snug">
          {q}
        </span>
        <span
          className={`flex-shrink-0 w-6 h-6 rounded-full flex items-center justify-center transition-all duration-200 ${
            open ? "bg-accent rotate-45" : "bg-panel"
          }`}
        >
          <svg
            className={`w-3 h-3 transition-colors ${open ? "text-white" : "text-ink-muted"}`}
      ...
      {open && (
        <p className="pb-4 text-sm text-ink-soft leading-relaxed pr-10">
          {a}
        </p>
      )}
    </div>
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      ...
      <nav className="flex items-center gap-2 text-xs text-ink-muted mb-6">
        ...
        <span>›</span>
        <span className="text-ink font-semibold">{t("faq.breadcrumb")}</span>
      </nav>

      {/* Header */}
      <div className="text-center mb-10">
        <div className="text-4xl mb-3">💬</div>
        <h1 className="text-3xl font-extrabold text-ink mb-2">{t("faq.title")}</h1>
        <p className="text-ink-soft text-sm max-w-md mx-auto leading-relaxed">
          {t("faq.subtitle")}{" "}
          <Link href="/contact" className="text-accent font-bold hover:underline">{t("faq.contactUsLink")}</Link>
        </p>
      </div>
      ...
      <div className="space-y-6">
        {faqs.map((section) => (
          <div key={section.category} className="bg-canvas rounded-card border border-line overflow-hidden">
            <div className="flex items-center gap-3 px-6 py-4 border-b border-canvas" style={{ backgroundColor: "#FAFAF8" }}>
              <span className="text-xl">{section.emoji}</span>
              <h2 className="font-extrabold text-ink">{section.category}</h2>
            </div>
            <div className="px-6">
      ...
      <div className="mt-10 p-6 rounded-card text-center" style={{ background: "linear-gradient(135deg, #EAF2F0 0%, #E8EEF4 100%)" }}>
        <p className="font-extrabold text-ink mb-1">{t("faq.stillQuestions")}</p>
        <p className="text-sm text-ink-soft mb-5">{t("faq.stillQuestionsBody")}</p>
        <div className="flex items-center justify-center gap-3 flex-wrap">
          ...
          <a
            ...
            className="inline-flex items-center gap-2 font-bold text-sm px-5 py-2.5 rounded-control border border-line text-ink-soft hover:border-accent hover:text-accent transition-colors"
          >
