...
function Stars({ n }: { n: number }) {
  return <span className="text-[#F0B840]">{"★".repeat(n)}<span className="text-[#DDD5CC]">{"★".repeat(5 - n)}</span></span>;
}
...
export default function ReviewsClient() {
  ...
  return (
    <div className="max-w-4xl">
      <h1 className="text-2xl font-extrabold text-[#2A2320] mb-1">Reviews</h1>
      <p className="text-[#9A8E88] text-sm mb-5">{rows.length} reviews · only customers who received a product can post one.</p>
      ...
      <div className="flex gap-2 mb-4">
        {(["all", "published", "hidden"] as const).map((f) => (
          <button key={f} onClick={() => setFilter(f)} className={`px-3 py-1.5 rounded-full text-xs font-bold transition-colors ${filter === f ? "bg-[#5E9E8C] text-white" : "bg-white border border-[#DDD5CC] text-[#5E5450]"}`}>
      ...
      <div className="space-y-3">
        {filtered.map((r) => (
          <div key={r.id} className={`bg-white rounded-2xl border p-4 ${r.status === "hidden" ? "border-red-200 opacity-70" : "border-[#DDD5CC]"}`}>
            <div className="flex items-start justify-between gap-3 flex-wrap">
              <div className="min-w-0">
                <div className="flex items-center gap-2 flex-wrap">
                  <Stars n={r.rating} />
                  <span className="font-bold text-[#2A2320] text-sm">{r.products?.name ?? `#${r.product_id}`}</span>
              ...
              <div className="flex gap-2 flex-shrink-0">
                <button onClick={() => setStatus(r.id, r.status === "hidden" ? "published" : "hidden")} className="text-xs font-bold text-[#5E9E8C] hover:underline">
                  {r.status === "hidden" ? "Publish" : "Hide"}
                </button>
                <button onClick={() => remove(r.id)} className="text-xs font-bold text-[#B03A3A] hover:underline">Delete</button>
              </div>
            </div>
            <p className="text-sm text-[#5E5450] mt-2 whitespace-pre-line">{r.body}</p>

            {r.admin_reply && !replyOpen[r.id] && (
              <div className="mt-3 rounded-xl bg-[#EAF2F0] border border-[#C8DDD8] p-3">
                <div className="flex items-center justify-between gap-2 mb-1">
                  <span className="text-xs font-extrabold text-[#3A6B5E]">Loov 🌿 replied</span>
                  <div className="flex gap-2 flex-shrink-0">
                    <button onClick={() => openReply(r)} className="text-[11px] font-bold text-[#5E9E8C] hover:underline">Edit</button>
                    <button onClick={() => removeReply(r.id)} className="text-[11px] font-bold text-[#B03A3A] hover:underline">Remove</button>
                  </div>
                </div>
                <p className="text-sm text-[#3A6B5E] whitespace-pre-line">{r.admin_reply}</p>
              </div>
            )}
            {!r.admin_reply && !replyOpen[r.id] && (
              <button onClick={() => openReply(r)} className="mt-2 text-xs font-bold text-[#5E9E8C] hover:underline">+ Reply as Loov</button>
            )}
            {replyOpen[r.id] && (
              <div className="mt-3 rounded-xl border border-[#5E9E8C] p-3 bg-[#F5FBF9]">
                <textarea
                  value={replyDraft[r.id] ?? ""}
                  onChange={(e) => setReplyDraft((prev) => ({ ...prev, [r.id]: e.target.value }))}
                  rows={2}
                  placeholder="Write a public reply as Loov…"
                  className="w-full rounded-lg border border-[#DDD5CC] px-3 py-2 text-sm outline-none focus:border-[#5E9E8C] resize-y"
                />
                <div className="flex gap-2 mt-2">
                  ...
                  <button
                    ...
                    className="text-xs font-bold text-[#5E5450] px-3 py-1.5 rounded-lg border border-[#DDD5CC]"
                  >
                    Cancel
                  </button>
                </div>
        ...
        {filtered.length === 0 && <p className="p-6 text-center text-sm text-[#9A8E88] bg-white rounded-2xl border border-[#DDD5CC]">No reviews {filter !== "all" ? `(${filter})` : "yet"}.</p>}
      </div>
    </div>
  );
}
