...
### 🛒 TEMU DÖNÜŞÜMÜ + ÇOKLU HESAP + NAVBAR CAM (17-18 Tem 2026) — TAMAMLANDI, HEPSİ PUSH EDİLDİ
Uzun bir oturum; kullanıcı demo-seç-uygula döngüsüyle ilerledi (her büyük UI değişikliği önce artifact demosuyla sunuldu, kullanıcı seçti, sonra uygulandı — bkz. `[[feedback-plan-before-changes]]`). **Önemli çalışma kuralı bu oturumda pekişti:** plansız/tahminle iş yapma → 16-17 Tem'de mobil hamburger 4 kez plansız yeniden yapılıp reddedilince kullanıcı sinirlendi; artık her değişiklik önce demo/soru ile onaylanıyor. Yapılanlar (hepsi tsc+60test+build temiz, canlıda):
- **Mobil hamburger — NİHAİ:** 4 reddedilen denemeden sonra kullanıcı 5 demoluk artifact'ten **"design 4 = sağdan kayan çekmece"** seçti → `MobileMenuDrawer.tsx` (blurlu arka plan, alt tab bar görünür kalır). `MobileMenuSheet.tsx`+`MobileMenu.tsx` silindi. Sonra çekmecenin en başına **kompakt profil kartı** (avatar+isim+e-posta+sağlayıcı rozeti) + **"Hesap Değiştir"** (inline hatırlanan hesap listesi) eklendi. Misafirse giriş satırı.
- **Ürün detay (PDP) Temu tarzı:** kenardan-kenara galeri + nokta/sayaç + köşe favori kalbi, fiyat en üstte/büyük + indirim çipi + "N ₾ tasarruf", ad küçük, tıklanabilir puan çipi (#reviews), sekme kaydırma-çubuğu gizlendi, kompakt güven şeridi, **Sepete Ekle + Hemen Al** (Hemen Al `loov_checkout_keys` ile SADECE o ürünü checkout'a götürür). Sahte aciliyet EKLENMEDİ (dürüst içerik ilkesi).
- **Ürünler sayfası (Koleksiyonumuz) Temu tarzı:** üstte arama kutusu + **soldan animasyonlu "Filtreler" çekmecesi** (fiyat/yaş/RENK İSİM ÇİPLERİYLE-yuvarlak değil/kumaş/sezon/indirim/son-görülen) + **Kategori ▾** + **Sırala ▾** dropdown'ları. Eski pill satırı + renkli-yuvarlak renk seçici + grid/liste toggle kalktı. Load More + alt bar korundu. `CategoryFilter.tsx` baştan.
- **Hesabım → Bento panosu:** kullanıcı 3 demodan "bento"yu seçti. Büyük ödül kartı (gerçek puan+seviye ilerleme), **canlı aktif sipariş** (fetchMyOrders+adım çubuğu), favori, bebek, hızlı karanlık-mod+dil, tam-genişlik hesap listesi + **"Hesaplarınız" switcher**. `/account/settings` sayfası kaldırılmıştı (bento'da inline).
- **Çoklu hesap (güvenli):** `src/lib/rememberedAccounts.ts` + `useRememberedAccounts` — TOKEN saklamadan (id/email/name/avatar/provider, max 5) cihazda hatırlar. Değiştirme = `/login?email=…&switch=1` ile GERÇEK tekrar-giriş (Supabase oturumu değiştirir). AuthContext girişte hatırlar. XSS başka oturum çalamaz. Planı `[[project-multi-account]]`.
- **/deals (İndirimde) özel kampanya sayfası (A1):** kırmızı hero + DÜRÜST geri sayım (gerçek `discountEndsAt`, yoksa gizli) + en büyük indirim spotlight + %10/20/30 çipleri + ProductCard ızgara. İndirimli ürün yoksa boş-durum. Navbar üst şeridinden "İndirimde"+"Son Görüntülenenler" KALDIRILDI (kalitesiz hissettiriyordu) → ikisi de hamburger çekmecesinde; İndirimde artık `/deals`. Sitemap'e eklendi. (Son Görüntülenenler özel sayfası için 2 demo sunuldu, kullanıcı henüz seçmedi → hâlâ `/products?recent=1`.)
- **Navbar üst şerit (mobil):** 4 pill (Ürünler/Paketler/Beden Kılavuzu/Blog) `justify-between`+küçük padding ile tek satıra sığdırıldı (390px'te taşma yok, ka için overflow-x güvenlik ağı).
- **Navbar frosted cam düzeltmesi:** iki ayrı `backdrop-blur` satırı → TEK sürekli cam yüzeyi (yansımalar sürekli) + aralarında **belirgin ayırıcı çizgi** (`border-t-2`+inset highlight); blur azaltıldı (lg→md, /75→/70); PC arama dropdown'ı artık camın ÖNÜNDE (ikinci blur katmanı kalkınca stacking düzeldi). PC+mobil, aydınlık+karanlık doğrulandı.
- **Güvenlik testi yapıldı:** admin 404, CSRF 403, auth 401, SQL-injection güvenli, promo sızıntı yok, yeni kodda service-role/XSS yok — temiz.
- **GORSEL-PLANI.md güncellendi:** v2.0 Nordic paleti/fontları + referans görsel kaynakları (Unsplash/Pexels/unDraw/DALL·E/remove.bg). Kullanıcı görselleri yarın hazırlayacak.

### 🩹 17 MADDELİK CANLI-TEST HATA RAPORU (16 Tem 2026, gece) — 16/17 TAMAMLANDI
Kullanıcı canlı kullanırken bulduğu 17 maddeyi tek tek raporladı ("goz boyama yapma", kök sebep istedi). Önce plan dosyası (`BUG-RAPORU-17-MADDE.md`) yazıldı, sonra sırayla uygulandı. Tam detay + doğrulama o dosyanın "✅ SONUÇ" bölümünde. Öne çıkan gerçek bug'lar (yüzeysel yama değil, kök sebep):
- **KRİTİK (#15+#17): sipariş iptali/yorum düzenleme kullanıcıyı "hesaptan atıyordu"** — gerçek sebep `requireVerifiedSession()` step-up kapısının `/api/orders` (cancel) ve `/api/reviews` (PATCH/DELETE) üzerinde de duruyor olmasıydı; `trusted_devices` çerezi (varsayılan 4 saat) dolunca 401 `otp_required` dönüp `/login?verify=1`'e fırlatıyordu — oturum bozulmuyordu ama tam bir "atılma" hissi veriyordu VE asıl işlem (iptal/düzenleme) hiç gerçekleşmiyordu. Bu iki rota artık sadece normal oturum+sahiplik kontrolü istiyor (adım-doğrulama sadece `account/delete`+`returns`'te kaldı — gerçekten yüksek riskli olanlar). Yan bulgu: `mapStatus()`'ta ham "pending" durumu hiç ele alınmıyordu, "Processing/Hazırlanıyor" gösteriliyordu — yeni gerçek `"Pending"` durumu eklendi (4 dilde).
- **#6/#7: "İndirimde" ürüne tıklayınca sayfa büzülüyordu** — kök sebep `QuickViewButton`/`CartDrawer`/`BundleQuickView`'ın üçünün de BAĞIMSIZ (referans-sayımsız) `document.body.style.overflow` kilidi kurması; biri kapanırken diğerinin kilidini siliyordu. Yeni paylaşılan `src/hooks/useBodyScrollLock.ts` (modül-seviyeli sayaç) üçüne de bağlandı. "Son Görüntülenenler" nav pill'i + ayrı bölüm kaldırıldı, yerine `CategoryFilter`'da gerçek bir filtre çipi (`/products?recent=1`) geldi.
- **#1: dil değiştirici mobil hamburger'da ekran dışına taşıyordu** — kök sebep pozisyon hesabının SADECE butonun sağ kenarına göre yapılması (masaüstünde buton sağda olduğu için hiç sorun çıkmıyordu). `useLayoutEffect` ile dropdown'ın gerçek genişliği ölçülüp boyaya kadar viewport'a kelepçeleniyor.
- **Yeniden tasarımlar:** `/account` (görsel istatistik kartları + gruplu menü, "amatör" hairline-grid+düz-liste yerine), yeni `/account/settings` sayfası (karanlık mod + dil — önceden hesap sayfasının sağ kolonunda gömülüydü), mobil hamburger tam bottom-sheet'e çevrildi (`MobileMenuSheet.tsx`, bebek adı/doğum tarihi hızlı-düzenleme eklendi), sepet mobilde Temu-tarzı sabit alt bar (Total + Checkout + genişleyen döküm — önceden toplamı görmek için tüm listeyi kaydırmak gerekiyordu), beden kılavuzu mobilde 5 kolonlu tablo yerine kart listesi.
- **Küçük düzeltmeler:** alt nav aktif sekmeye kayan pill+animasyon, günlük makale breadcrumb'ı "← Günlüğe dön" tek linkine indirgendi, beden kılavuzu breadcrumb'ı kaldırıldı, `/products` ghost yükleme ekranı gerçek kart/filtre yapısıyla eşleşecek şekilde yeniden yazıldı, mobil kategori pilleri küçültüldü, "iade için X gün kaldı" baskıcı metni kaldırıldı.
- **Kod dışı (#3, kullanıcı yapacak):** Google ile girişte "Loov" yerine supabase proje URL'i görünmesi — Google Cloud Console → OAuth consent screen → App name alanının doğru girilmemiş/doğrulanmamış olmasından kaynaklanıyor, kodda düzeltilecek bir şey yok.
- Doğrulama: `tsc`+60 test+`next build` (45 route, `/account/settings` dahil) temiz; Playwright ile 390×844 canlı test (dil değiştirici artık ekranda, hamburger sheet açılıyor, indirim banner'ı + geri dönüşte büzülme yok, sepet sabit bar aç/kapa çalışıyor, beden kılavuzu kart görünümü, günlük geri linki) — konsol hatası sıfır. **Commit/push henüz yapılmadı.**

### 📋 TÜM MD DOSYALARI DENETİMİ + SEO/ADMIN EK İŞLER (16 Tem 2026) — TAMAMLANDI, PUSH EDİLMEDİ
Kullanıcı "bütün MD dosyalarını kontrol et, bilinçli yapmadığın her şeyi ortaya çıkar, rapor ver, şu an yapabileceklerini yap" dedi. Explore ajanıyla kök dizindeki tüm MD dosyaları (BACKEND_SETUP/buglar/CEVIRI-KONTROL-KA+BLOG/DENETIM-RAPORU/GORSEL-PLANI/GUVENLIK-RAPORU/PLAN-11-MADDE/PLAN-15-MADDE/RAKIP-ANALIZI/README/"Selam claude"/YAPILACAKLAR-PLAN) baştan sona okunup kod tabanıyla çapraz doğrulandı (birçoğu bayat — daha sonraki oturumlarda çözülmüş ama dosyaya yazılmamış). Sonuç 5 kovaya ayrıldı: (1) kullanıcının iş bilgisi/içerik gerektirenler (gerçek fotolar, WhatsApp numarası, hello@loov.ge, ürün adı/açıklaması KA/RU/TR içerik girişi, ka/ru/tr native inceleme), (2) kullanıcının kendi yapması gerekenler (SQL migration'ları — promos/addresses/variant-stock/product-i18n/trusted-devices/cart-wishlist/stock-notifications/rate-limits/loyalty-atomic/product-search/discount-timer/product-video.sql'in ÇALIŞTIRILDI onayı yok; Supabase Frankfurt göçü; Twilio; GA4/Meta Pixel; BOG/TBC), (3) kullanıcı kararı gerekenler (npm audit'teki 2 orta seviye açık — postcss+esbuild, ikisi de sadece `--force` ile düzeliyor ve Next.js'i çok eski bir sürüme düşürüyor, ÖNERİ: düzeltme yapma, upstream yaması bekle; hediye paketi gerçek fiyatı gibi ayar seçimleri), (4) **saf kod işi — hemen yapıldı**, (5) zaten çözülmüş ama dosyaya işlenmemiş (çoğu 15 Tem'deki mobil/dönüşüm/dil-switcher turlarında sessizce halloldu).
- **`/category/[kategori]` SEO iniş sayfaları** (YENİ) — önceden kategoriler sadece `/products?cat=X` query param'ıydı (crawler'lar için değersiz); artık her kategori (body/blanket/set/towel/romper/bag) gerçek, indexlenebilir, kendi `<title>`/meta description'ı olan bir URL. `/products/[slug]` (ürün detay) ile aynı seviyede dinamik segment ÇAKIŞACAĞI için (Next.js aynı seviyede farklı isimli iki dinamik segmente izin vermiyor) ayrı `/category/` kökü seçildi. Sitemap'e otomatik eklendi (katalogdaki gerçek kategorilerden türetiliyor, hardcode yok), ana sayfa kategori grid'i + footer linkleri yeni URL'e güncellendi. Yeni `meta.category.title/description` anahtarı ×4 dil (ka/ru/tr makine kalitesinde, `CEVIRI-KONTROL-KA.md` bölüm 26'ya not düşüldü).
- **Admin siparişler → CSV export** (YENİ) — DENETIM-RAPORU'nun "CRM/bulk-export" boşluğunun küçük ama hemen değer veren bir dilimi: `/admin/orders`'a "⬇ Export CSV" butonu, aktif durum filtresini uyguluyor, Excel'de Gürcüce/Kiril isimlerin bozulmaması için UTF-8 BOM'lu.
- **Ölü/yanıltıcı isim temizliği** — `src/lib/mockOrders.ts` artık hiç mock veri içermiyordu (gerçek sipariş tipleri + durum stilleri) ama ismi hâlâ "mock" diyordu (DENETIM Bölüm C #3'te bayat kalmış bir bulgu) → `src/lib/orderTypes.ts`'e yeniden adlandırıldı, 5 import güncellendi.
- **buglar.md #5 (PDP'de "hayalet kaydırma butonu") doğrulandı — zaten ÇÖZÜLMÜŞ**: 9-10 Tem'deki `overflow-y-hidden` fix'i (tab çubuğundaki CSS quirk) tam bu şikayetle eşleşiyor, kod bunu doğruluyor; buglar.md dosyası güncellenmedi ama bug canlıda yok.
- **`npm audit` ilk kez çalıştırıldı**: 2 farklı orta-seviye açık (esbuild — sadece dev sunucusu, prod'a hiç girmiyor; postcss — Next.js'in KENDİ gömülü kopyası, bizim kullandığımız postcss değil). İkisi de `--force` olmadan düzelmiyor ve düzeltme Next.js'i `9.3.3`'e (!) düşürüyor — YAPILMADI, kullanıcı onayı olmadan yapılmayacak.
- **Bilinçli yapılmayan (büyük/riskli, ayrı oturum)**: next/image migration (44 dosya, gerçek fotolar gelene kadar ertelenmesi zaten mantıklı), locale-URL/hreflang (~40 sayfa, tekrar tekrar "ayrı büyük oturum" olarak işaretlenmiş), storage'daki TARİHSEL yetim dosyaların toplu temizliği (13 Tem'de sadece "ileriye dönük" temizlik yapıldı — geçmiş yetimler için bucket'ın TAMAMINI listeleyip her referansla karşılaştırmak gerekiyor, yanlış dosya silme riski taşıyor, dry-run/önizleme tasarımıyla ayrı yapılmalı), referral/tavsiye programı (yeni tablo+UI gerektiren büyük özellik), best-sellers vitrini (gerçek sipariş verisi artık var, orta ölçekli iş), terk edilmiş sepet + satın-alma-sonrası-yorum hatırlatma mailleri (cron/otomasyon altyapısı gerektiriyor, önceki oturumlarda da aynı gerekçeyle ertelenmişti).
- Doğrulama: tsc + 60 test + `next build` + prod sunucuda canlı curl (6 kategori sayfası 200, geçersiz kategori 404, ürün detay sayfası regresyonsuz, sitemap'te 6 kategori URL'i, KA başlık/meta doğru render) hepsi temiz.

### 🛠️ ADMIN PANELİ 1 SÜRÜM YÜKSELTME (16 Tem 2026) — TAMAMLANDI, PUSH EDİLDİ
Kullanıcı "/admin'i internette araştır, hardcoded-ama-alti-boş alanları bul, ürün/paket ekranı parity'sini kapat, görsel cila çek, kaydet butonu ekle, error açıklaması eksikse ekle" dedi. Detaylar `[[project-admin-panel-levelup]]` memory kaydında. Özet: Products ekranına açık Save butonu + dirty-tracking + `card_color`/`emoji` editörü eklendi; Bundle editörüne bileşen-stok görünürlüğü + editable promo yüzdesi eklendi; admin genelindeki TÜM `alert()` çağrıları inline hata banner'ına çevrildi (bu arada ReviewsClient'ta optimistic-update rollback eksikliği ve RevenueChart'ta yakalanmayan fetch hatası gibi 2 gerçek bug bulunup düzeltildi); AdminShell'e `bg-panel` sayfa arkaplanı + `bg-canvas` kartlarla gerçek derinlik kazandırıldı. `tsc`+60 test+build temiz. Commit `89a999d`, push edildi.

### 💰 DÖNÜŞÜM ÖZELLİKLERİ ARAŞTIRMASI + TAMAMLAMA (15 Tem 2026) — TAMAMLANDI, PUSH EDİLMEDİ
Kullanıcı "büyük siteler ne yapıyor, hepsini ekle, yarım kalanı tamamla, satışı artır" dedi. Web araştırması (Baymard/NN Group + 2026 e-ticaret dönüşüm kaynakları) + kod tabanı denetimi (Explore ajanı) yapıldı. **Önemli karar:** sahte "az önce satın aldı" / uydurma aciliyet EKLENMEDİ — AB Omnibus Direktifi artık bunu ciro'nun %4'üne kadar cezayla yasaklıyor, ve sitenin zaten yerleşik "dürüst içerik" ilkesiyle (sahte testimonial/ekip/sertifika iddialarının önceden kaldırılmış olması) çelişir. Bunun yerine GERÇEK veriye dayalı boşluklar dolduruldu:
- **Gerçek "birlikte alınanlar"** (`getFrequentlyBoughtWith`, `src/lib/db/products.ts`) — önceden PDP'deki "You Might Also Like" sadece kategori+doldurma idi (sahte değil ama hedefsizdi); artık ÖNCE gerçek sipariş ortak-alım verisine bakıyor (`order_items` tablosunda aynı siparişte geçen ürünler, service-role ile — RLS anonim istatistiksel sorguyu engellediği için), yetersizse (yeni ürün, az sipariş) kategori+dolduma'ya zarif düşüyor. Aynı fonksiyon satın alma sonrası öneri için de kullanılıyor.
- **Satın alma sonrası öneri** (checkout "Sipariş Alındı" ekranı) — önceden hiç yoktu, şimdi `useSuggestedProducts` ile gerçek ürünler gösteriliyor ("Beklerken Bunlar da Hoşunuza Gidebilir").
- **Checkout'ta promosyon kodu girişi** — önceden SADECE sepet sayfasında girilebiliyordu, checkout'a sepetten atlarsa (veya kod hiç girmeden geçtiyse) kod girme imkânı yoktu; artık review adımında da aynı doğrulama/hata mesajlarıyla girilebiliyor.
- **Yorumlarda sıralama + yıldıza göre filtre** — önceden düz liste + dekoratif (tıklanamaz) yıldız dağılım çubuğuydu; artık dağılım çubukları TIKLANABİLİR filtre oldu + yeni/eski/yüksek-puan/düşük-puan sıralama dropdown'ı eklendi (client-side, API değişikliği gerekmedi).
- **Yan bulgular (aynı denetimde bulunan i18n sızıntıları):** PDP'deki "You Might Also Like" başlığı ve breadcrumb'taki "Home"/"Products" hardcoded İngilizceydi (çeviri anahtarı zaten VARDI, sadece kullanılmamıştı) — düzeltildi.
- Zaten TAM/iyi olduğu doğrulanan (dokunulmadı): checkout adım göstergesi, sepet ücretsiz-kargo ilerleme çubuğu (hem /cart hem drawer'da), PDP güven rozetleri (Sepete Ekle butonunun hemen altında).
- **Bilinçli ertelenen:** ürün karşılaştırma özelliği (bebek giyimi için düşük öncelik, elektronik/karmaşık ürünlerde daha değerli), gerçek kargo firması takibi (Georgia Post API hesabı gerekiyor — dış bağımlılık), terk edilmiş sepet e-postası (cron + otomasyon altyapısı gerektiren ayrı bir proje, istenirse yapılabilir).
- Doğrulama: tsc+60 test+build temiz; Playwright ile canlı test edildi (PDP'de gerçek "birlikte alınanlar" render oluyor, checkout review adımında promosyon kutusu doğru konumda).
- **Kullanıcı "şimdilik canlıya alma" dedi — commit edildi, push YAPILMADI.**

### 📱 MOBİL UX CHECK-UP (15 Tem 2026) — TAMAMLANDI
Kullanıcı "büyük şirketler ne yapıyor, mobil kullanıcı deneyimi nasıl olmalı" diye araştırma + eksiksiz düzeltme istedi (site trafiğinin neredeyse tamamı mobil). Web araştırması (Baymard/NN Group dokunma-hedefi standartları, thumb-zone/sticky-CTA konvansiyonları, safe-area-inset) + siteye canlı Playwright denetimi yapıldı.
- **En büyük somut bulgu:** CLAUDE.md'nin geçmişinde "MobileBottomNav — 4 tab" diye bahsedilen bileşen **kod tabanında hiç yoktu** (Nordic redesign'da düşmüş, dokümantasyon güncellenmemiş — bkz. `[[feedback-verify-claudemd-against-code]]` memory kaydı, bir önceki turda ben de bu yanlış varsayıma dayanarak hamburger'dan "Ana Sayfa"yı kaldırmıştım). **`MobileBottomNav.tsx` yeniden inşa edildi:** Ana Sayfa/Ürünler/Favoriler/Sepet/Hesabım, rozet sayaçları, aktif-rota vurgusu, `env(safe-area-inset-bottom)` desteği; `/checkout` ve `/admin`'de gizli (FooterGate'in aynı route'ları).
- Yeni `viewport` export (`viewportFit: "cover"`) — safe-area-inset'in çalışması için şart.
- Tüm diğer sabit-alt elemanlar (CartToast/BackToTop/CookieConsent/WhatsAppButton/PDP sticky "Sepete Ekle" barı) yeni nav'ın üstüne `bottom: calc(4.5rem + env(safe-area-inset-bottom))` ile taşındı — önceden hepsi düz `bottom-6` idi, yeni nav'ın ALTINDA/İÇİNDE kalırlardı.
- Navbar mobil ikonları (arama/favori/hamburger/tema) 36px→44px (Apple/Google dokunma-hedefi minimumu) — bunu yaparken satır 375-390px ekranda taşmaya başladı, çözüm olarak **favoriler+sepet ikonları mobilde kaldırıldı** (`md:flex`) çünkü artık alt nav'da aynı rozetlerle zaten var — hem taşmayı çözdü hem tekrarı giderdi.
- Denetim sırasında bulunan yan hatalar: CookieConsent'te karanlık modda bozuk eski renk kalıntısı (CartToast'takiyle aynı bug) + navbar sepet rozetinde aynı sorun, ikisi de düzeltildi; footer alt-bar yasal linkleri (Gizlilik/Şartlar/Erişilebilirlik/İletişim) dar ekranda birkaç piksel taşıyordu → `flex-wrap` eklendi.
- Doğrulama: 390×844 gerçek viewport'ta ana akışların (anasayfa/ürünler/PDP/sepet/favoriler/checkout/hesap) TAMAMI taranıp yatay taşma sıfırlandı; tsc+60 test+build temiz.
- **Bilinçli ertelenen (daha büyük efor gerektirir, bu turda yapılmadı):** ürün fotoğrafları hâlâ düz `<img>` (Next.js `<Image>` lazy-loading/otomatik boyutlandırma yok — mevcut CLAUDE.md'de zaten "🔵 büyük kod" olarak işaretli bilinen boşluk); site genelinde `w-9 h-9` (36px) küçük ikon butonları (ProductCard'ın QuickAddButton'ı gibi) hâlâ 44px altı — sadece en yüksek trafikli navbar ikonları + wishlist kalp'i büyütüldü, kart-içi butonlara dokunulmadı (yoğun grid'lerde görsel dengeyi bozma riski).

### 🔧 15-MADDE GERİ BİLDİRİM TURU + RE-AUDIT — TAMAMLANDI (15 Tem 2026)
v2.0.0 canlıya çıktıktan sonra kullanıcının 15 maddelik canlı-test geri bildirimi `PLAN-15-MADDE.md`'de Batch A-F halinde işlendi, `redesign-nordic`'e commit edilip **main'e merge edildi (loov.ge'de yayında)**. Kullanıcı sonra 3 maddenin (üst bar zenginleştirme, puan çipi, ghost/skeleton ekranları) hâlâ yanlış olduğunu bildirdi ve "sadece söylediklerimle kalma, 15 maddenin TAMAMINI sen kontrol et" dedi (bkz. `[[feedback-proactive-audit]]` memory kaydı) → 15 madde tek tek gerçek koddan yeniden okunup doğrulandı, sonuç `PLAN-15-MADDE.md`'nin sonundaki "🔁 RE-AUDIT SONUCU" bölümünde. Re-audit **4 madde gerçekten bozuk** buldu; plan `C:\Users\Miko\.claude\plans\selam-claude-bizim-wep-sparkling-bee.md`'de yazıldı ve kullanıcının "devam edelim kalan maddeleri de hallet" onayıyla **aynı gün uygulandı** (tam detay `PLAN-15-MADDE.md`'nin "✅ 4 KALAN MADDE UYGULANDI" bölümünde):
1. **Navbar üst şerit yeniden kuruldu** — tekil kategori pilleri (zaten `/products`'ta `CategoryFilter`'da var) kaldırıldı; şerit artık Ürünler → Paketler → **İndirimde** (yeni, `/products?deal=1`, `CategoryFilter`'a `dealOnly` filtresi) → **Son Görüntülenenler** (yeni, hesap-dropdown deseninde popover, mevcut `recentProducts` verisini kullanıyor) → Blog.
2. **Puan çipi** (`Navbar.tsx`) — çerçeveli kutu kaldırıldı, sağdaki gerçek komşularıyla (arama/kalp/tema/hesap) aynı çerçevesiz `hover:bg-panel` ailesine sokuldu.
3. **Mobil hamburger/arama satırı kapanış animasyonu** — `globals.css`'e `loov-fade-down` eklendi, her iki blok `useDelayedUnmount` ile sarıldı; kapanış artık açılışın simetriği.
4. **Ghost/skeleton ekranları** (`src/components/GhostRows.tsx`) — `variant` prop'u eklendi (`rewards`/`security`/`notifications`/`list`), her hesap sayfası artık kendi gerçek şekline (bakiye kartı/form alanları/toggle satırları/foto+metin satırı) yakın bir iskelet gösteriyor.

Doğrulama: `tsc`+`npm test` (60/60)+`next build` temiz, Playwright ile canlı test edildi. Diğer 11 madde (#2,3,5,6,7,9,10,11,12 kısmen, 15) zaten kod okunarak doğru/tamam olarak doğrulanmıştı — onlara dokunulmadı. **Not:** commit/push henüz yapılmadı, kullanıcı onayı bekleniyor (önceki turlarda olduğu gibi).
