...
- **⚠️ Kullanıcı çalıştıracak (yeni, TEYİT EDİLMEDİ):** `supabase/cart-wishlist.sql` · `supabase/stock-notifications.sql` (ikisi de çalıştırılana dek zarif düşer — sepet cihazlar-arası senkron olmaz, back-in-stock kaydı tutulmaz ama hata vermez).
